...
COLLECTION_REG = "event_registrations"
...
async def register_for_event(event_id: str, user_id: str):
    existing = await db[COLLECTION_REG].find_one({
    ...
    result = await db[COLLECTION_REG].insert_one(reg_data)
    new_reg = await db[COLLECTION_REG].find_one({"_id": result.inserted_id})
    return serialize_registration(new_reg)

async def checkin_registration(registration_id: str):
    reg = await db[COLLECTION_REG].find_one({"_id": ObjectId(registration_id)})
    if not reg:
        raise HTTPException(status_code=404, detail="Registration not found")
    await db[COLLECTION_REG].update_one(
